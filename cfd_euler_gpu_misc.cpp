...
#pragma omp target data \
	map(rho[0:total_size]) \
	map(rhou[0:total_size]) \
	map(rhov[0:total_size]) \
	map(E[0:total_size]) \
...
//		#pragma omp target teams distribute parallel for reduction(+:cupdate_count) collapse(2)
		for (int i = 1; i <= Nx; i++){
            for (int j = 1; j <= Ny; j++){
...
//		#pragma omp target teams distribute parallel for reduction(+:copy_count) collapse(2)
        for (int i = 1; i <= Nx; i++){
            for (int j = 1; j <= Ny; j++){
...
//		#pragma omp target teams distribute parallel for reduction(+:kinetic_count,total_kinetic) collapse(2) 
        for (int i = 1; i <= Nx; i++) {
            for (int j = 1; j <= Ny; j++) {
